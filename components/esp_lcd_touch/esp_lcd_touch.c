...
{
    assert(tp != NULL);
    assert(touch_data != NULL);
    assert(point_num != NULL);

    if (max_point_num == 0 || max_point_num > CONFIG_ESP_LCD_TOUCH_MAX_POINTS) {
    ...
    bool touched = esp_lcd_touch_get_coordinates(tp, x, y, strength, &points, max_point_num);
    if (!touched) {
        *point_num = 0;
        return ESP_OK;
    }
    ...
    for (uint8_t i = 0; i < points; i++) {
        touch_data[i].x = x[i];
        touch_data[i].y = y[i];
        touch_data[i].strength = strength[i];
