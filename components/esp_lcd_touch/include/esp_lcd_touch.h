...
/**
 * @brief Single touch point data (compatibility with newer esp_lcd_touch APIs)
 */
typedef struct {
    uint16_t x;         /*!< X coordinate */
    uint16_t y;         /*!< Y coordinate */
    uint16_t strength;  /*!< Touch strength */
    uint8_t track_id;   /*!< Tracking ID, monotonically assigned within a sample */
...
/**
 * @brief Get touch point data as a struct array (compatibility helper)
 *
 * @param tp: Touch handler
 * @param touch_data: Output buffer for touch points
 * @param point_num: Output number of valid points in `touch_data`
 * @param max_point_num: Size of `touch_data` buffer
 *
 * @return
 *      - ESP_OK on success
